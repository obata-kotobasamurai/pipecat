...
class TestProcessWordMisroutedSentencePunctuation(unittest.TestCase):
    ...
    def test_alnum_passthrough_still_appends_to_context(self):
        """Words from an unknown context are stale in 1.4.0 and must be dropped."""
        seq = _seq()
        result = seq.process_word("hello", pts=1, context_id="ctx-unknown")
        self.assertEqual(result, [])
...
class TestProcessWordMidFrameStandalonePunctuation(unittest.TestCase):
    ...
    def test_skipped_symbol_frame_still_emitted_for_word_timestamps(self):
        ...
        word_frames = [f for f in result if isinstance(f, TTSTextFrame)]
        progress_frames = [f for f in result if isinstance(f, AggregatedTextProgressFrame)]

        self.assertEqual(len(word_frames), 1)
        self.assertEqual(len(progress_frames), 1)
        self.assertEqual(word_frames[0].text, "、")
        self.assertFalse(word_frames[0].append_to_context)
