...
class SmallWebRTCTransport(BaseTransport):
    ...
    async def _on_app_message(self, message: Any, sender: str):
        ...
        Intercepts DTMF messages (``{"type": "dtmf", "digit": "1"}``) and
        converts them to an ``InputDTMFFrame`` for pipeline processing. All
        other messages are forwarded as ``InputTransportMessageFrame`` via
        ``push_app_message()``.
        """
