...
class TTSService(AIService):
    ...
    async def _maybe_reset_word_timestamps(self, context_id: str | None = None):
        ...
        Args:
            context_id: The audio context that just completed. When it belongs to a
                TTSSpeakFrame utterance (e.g. a pre-tool filler spoken while a tool
                runs), the deferred LLMFullResponseEndFrame is NOT emitted and
                ``_llm_response_started`` is left untouched: the pending LLM
                response's own audio context has not played yet, and consuming the
                flag here would close the assistant turn early — the LLM's spoken
                words would then accumulate without an End frame and be silently
                dropped from the context on the next interruption.
        """
        await self.reset_word_timestamps()
