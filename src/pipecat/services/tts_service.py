...
class TTSAudioContextStats:
    ...
    def tail_text_after_audio(self) -> str:
        ...
        tail = [
            entry for entry in self._timestamped_words if entry.timestamp > self.audio_seconds
        ]
        if not tail:
            return ""
...
class TTSService(AIService):
    ...
    async def _maybe_push_truncated_context_error(
        self, context_id: str, end_reason: str
    ) -> bool:
